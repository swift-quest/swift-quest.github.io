...
function attack(scene: Scene, target: Character, damage: number, noDamageMessage: string, whole: boolean = false): Message[] {
  ...
  if (damage > 0) {
    messages.push(new Message(target.name.concat(" took ").concat(String(damage)).concat(" damage!"), () => {
...
class MagicShieldSpell extends Spell {
  constructor() {
    super("Magic Shield", new Color(255, 255, 255), 13, false);
  }
...
class Spells {
  static healing = new RestorationSpell("Healing", new Color(255, 255, 255), 5, false, 80);
  static resurrection = new RestorationSpell("Resurrection", new Color(255, 255, 255), 15, false, 999, true);
  static fireball = new AttackSpell("Fireball", new Color(254, 75, 38), 5, false, 70);
  static inferno = new AttackSpell("Inferno", new Color(254, 75, 38), 10, false, 130);
  static blizzard = new AttackSpell("Blizzard", new Color(38, 75, 254), 9, true, 70);
  static thunderbolt = new AttackSpell("Thunderbolt", new Color(254, 254, 75), 8, false, 100);
  static tempest = new AttackSpell("Tempest", new Color(127, 127, 127), 12, true, 95);
...
class AttackAction extends Action {
  ...
  perform(scene: Scene): Message[] {
    let messages: Message[] = [new Message(this.character.name.concat(" attacks."))];
    let damage = Math.max(0, Math.floor((this.character.attack - this.target.defense * Math.random()) / 2 / (this.target.isDefending ? 2 : 1)));
    for (let message of attack(scene, this.target, damage, " took no damage!")) {
...
class SpellAction extends Action {
  ...
  perform(scene: Scene): Message[] {
    ...
    let sceneSnapshot = scene.clone();
    let messages = [new Message(this.character.name.concat(" casts ").concat(this.spell.name).concat("."), () => { updateStatusBox(sceneSnapshot); return Promise.resolve(undefined); }, () => {
...
class SummonAction extends Action {
  ...
  perform(scene: Scene): Message[] {
    ...
    let sceneSnapshot = scene.clone();
    let messages = [
      new Message(this.character.name.concat(" calls for backup."), () => { updateStatusBox(sceneSnapshot); return Promise.resolve(undefined); }),
...
class PlayerCharacter extends Character {
  decideAction(scene: Scene): Promise<Action> {
    let thiz = this;
    return select([new SelectionItem<string>("Attack"), new SelectionItem<string>("Magic", this.spells.length == 0), new SelectionItem<string>("Defend")], this.name, true).then_((actionName) => {
      switch (actionName) {
        case "Attack": {
...
function startBattle(): Promise<void> {
  ...
  const scene = new Scene(
    ...
    new Party([
      new NonPlayerCharacter("Archfiend", 999, 99, 185, 58, 61, [Spells.inferno, Spells.blizzard, Spells.tempest], true, (character, scene) => {
      ...
      apply(new NonPlayerCharacter("Dark Knight", 250, 0, 181, 93, 73, [], true, (character, scene) => {
      ...
      apply(new NonPlayerCharacter("Demon Priest", 180, 99, 121, 55, 59, [Spells.healing, Spells.resurrection, Spells.magicShield], true, (character, scene) => {
        let spellsAvailable = character.spells.filter((spell) => character.mp >= spell.mp);
        if (Math.random() < 0.5) {
  ...
  updateStatusBox(scene);
  return showMessages([new Message(scene.enemy.members[0].name.concat(" appears."))]).then((value) => {
    return scene.performBattle();
  }).then((winner) => {
    if (winner == scene.friend) {
      winCount++;
      if (isBattleStandalone) {
        return select([new SelectionItem("Again")]).then_((item) => {
          clearSelections();
          return startBattle();
        });
      } else {
